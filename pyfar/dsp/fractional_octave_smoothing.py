...
class FractionalSmoothing:
    ...
    def integration_limits(self):
        ...
        freq_bins_up = np.array([
                        np.concatenate((k[:-1]+.5, np.array([k_U])))
                        for k, k_U in zip(freq_bins, cutoff_up)])
        # Divide by k and calc log:
        freq_bins_low /= self._disc_freq_bins
        freq_bins_up /= self._disc_freq_bins
        Phi_l = np.array([np.log2(i) for i in freq_bins_low])
        Phi_u = np.array([np.log2(i) for i in freq_bins_up])
        self._Phi = Phi = np.array([Phi_l, Phi_u])
